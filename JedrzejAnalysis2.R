...
data_sham = data %>% filter(!is.na(PC_Sham)) %>% mutate(Condition = "OverrideInput", PCValue = (PC_Sham-1)/6)
data_mf = data %>% filter(!is.na(PC_AF)) %>% mutate(Condition = "MediateFailure", PCValue = (PC_AF-1)/6)
data_as = data %>% filter(!is.na(PC_AS)) %>% mutate(Condition = "AugmentSuccess", PCValue = (PC_AS-1)/6)

data_comb = data_sham %>% bind_rows(data_mf) %>% bind_rows(data_as)
...
fig <- vistemplate %>% add_trace(data = data_comb, x=~as.factor(Condition), y=~jitter(PCValue,amount=.03), jitter=.35, color=I('darkgray'), width=1, points='all', pointpos=0, type="violin", soanmode="soft", bandwidth=.09) %>%
...
####################################
FRdata_sham = data %>% filter(!is.na(FR_Sham)) %>% mutate(Condition = "OverrideInput", FRValue = (FR_Sham-1)/6)
FRdata_mf = data %>% filter(!is.na(FR_AF)) %>% mutate(Condition = "MediateFailure", FRValue = (FR_AF-1)/6)
...
vistemplate %>% add_trace(data = FRdata_comb, x=~as.factor(Condition), y=~jitter(FRValue,amount=.03), jitter=.35, color=I('darkgray'), width=1, points='all', pointpos=0, type="violin", soanmode="soft", bandwidth=.09) %>%
...
#Means for PC between conditions in regard to PAMs
summary(PC_PAM_data)
...
#Means for FR between conditions in regard to PAMs
FR_PAM_data<-FR_PAM_data[-c(1:16),]
